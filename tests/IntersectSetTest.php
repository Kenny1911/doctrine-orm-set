...
use Kenny1911\Doctrine\Set\IntersectSet;
use Kenny1911\Doctrine\Set\QuerySet;
use Kenny1911\Doctrine\Set\Tests\Entity\Foo;
use PHPUnit\Framework\TestCase;

final class IntersectSetTest extends TestCase
{
    /** @var IntersectSet */
    private $set;
    /**
    ...
    {
        $this->assertSame(
            'SELECT f.id FROM Foo f WHERE f.id IN (:query1Ids) INTERSECT SELECT f.id FROM Foo f WHERE f.id IN (:query2Ids)',
            $this->set->getSql()
        );
    }
    ...
    public function testGetParameters(): void
    {
        ...
        $this->assertSame(
            ['query1Ids' => [1,2,3], 'query2Ids' => [3,4,5]],
            $parameters
        );
    }
